...
class SentiAnalyzer:
    ...
    def probWordPositiveAndNegative(self, sentidata, word, idxWord):
        ...
        pointedWord = word[idxWord] #특정 단어 선택
        reviews = [int(float(row[-1])) for row in sentidata] #row[-1]: 1 if positive, 0 negative
        occurrence = [int(float(row[idxWord])) for row in sentidata] #row[idxWord]: 1 if 특정단어가 row(review)에 있는 경우, 0 otherwise
        ...
        for i in range(len(occurrence)):
            positive = positive + reviews[i]*occurrence[i]
        ...
        negative = 0
        for i in range(len(occurrence)):
            negative = negative + (1-reviews[i])*occurrence[i]
